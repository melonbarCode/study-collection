...
//나의 문제풀이
function solution(prince = 0, k = 0) {
  let stack = Array(prince)
    .fill(1)
    .map((v, i) => i + 1);
  ...
  while (stack.length > 1) {
    ...
    for (let i = 0; i < stack.length; i++) {
      if (k - (rotate * stack.length + i + 1) === 0) {
        stack.splice(i, 1);
        const temp = stack.slice(i);
        stack = [...temp, ...stack.slice(0, i)];
        break;
      }
...
//다른 문제풀이 이게 낫네
function solution2(prince = 0, k = 0) {
  let queue = Array.from({ length: prince }, (_, i) => i + 1);
  let answer = 0;
