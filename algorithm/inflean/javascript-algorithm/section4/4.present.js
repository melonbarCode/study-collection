...
//나의 문제풀이
function solution(n = [0, 0], goods = [[0, 0]]) {
  const [num = 0, budget = 0] = n;
  let answer = 0;
  goods.sort((a, b) => a[0] + a[1] - (b[0] + b[1]));

  for (let i = 0; i < goods.length; i++) {
    let tempBudget = budget - (goods[i][0] + goods[i][1]);
    let cnt = 1;
    for (let j = 0; j < goods.length; j++) {
      if (goods[j][0] + goods[j][1] > tempBudget) break;
      if (goods[j][0] + goods[j][1] <= tempBudget) {
        tempBudget -= goods[i][0] + goods[i][1];
        cnt++;
      }
    }
